...
def main(model_args, data_args, training_args, additional_args, model_cls, trainer_cls, jupyter=False):    
    ...
    if training_args.do_eval:
        ...
        if additional_args.plotting_logits:
            ...
            plt.grid(True)
            plt.savefig("boxplot_topk_rank_eval" + data_args.dataset_name + "_"+model_args.model_name_or_path +".png")
...
if __name__ == "__main__":
    ...
    if not additional_args.plotting_logits:
        ...
        plt.plot(range(24), mean_block_confidence, label='confidence', color='midnightblue')
        plt.plot(range(24), block_k_metric, label='f1', color='red')
        plt.title('Confidence and f1 over blocks')
        plt.xlabel('Blocks')
